...
function clearForm() {
    ...
    // Destruimos el gráfico si existe
    if (chartInstance) {
        chartInstance.destroy();
        chartInstance = null;
    }

    // Desactivamos el botón de calcular
...
function calculate() {
    ...
    if (category === 'cinematica') {
        if (subcategory === 'mru') {
            ...
            const tUnit = document.getElementById('tUnit').value;
            const vfUnit = document.getElementById('vfUnit').value;
            const g = 9.81; // Aceleración debido a la gravedad en m/s² (positiva hacia abajo)
            ...
            // Convertimos las unidades a las unidades base
            v0 = v0 * conversions.velocidad[v0Unit]; // Convertimos v0 a m/s (puede ser negativa)
            h = h !== null ? h * conversions.longitud[hUnit] : null; // Convertimos h a metros
            t = t !== null ? t * conversions.tiempo[tUnit] : null; // Convertimos t a segundos
            vf = vf !== null ? vf * conversions.velocidad[vfUnit] : null; // Convertimos vf a m/s

            // Ecuaciones de caída libre teniendo en cuenta v0 (positiva hacia abajo, negativa hacia arriba):
            // 1. vf = v0 + g * t
            // 2. h = v0 * t + (1/2) * g * t² (h es la altura inicial, y decrece con el tiempo)
            // 3. vf² = v0² + 2 * g * h
            ...
            // Si v0 es negativa (lanzamiento hacia arriba), calculamos el tiempo hasta la altura máxima
            if (v0 < 0) {
                timeToMaxHeight = -v0 / g; // Tiempo hasta que la velocidad vertical es 0
                maxHeight = h + v0 * timeToMaxHeight + 0.5 * g * timeToMaxHeight * timeToMaxHeight; // Altura máxima
                if (maxHeight < 0) {
            ...
            if (vf === null && t !== null) {
                // Si no se da vf pero sí t, usamos: vf = v0 + g * t
                vf = v0 + g * t;
                // Calculamos h si no se dio
                if (h === null) {
                    h = v0 * t + 0.5 * g * t * t;
                }
            } else if (t === null && vf !== null) {
                // Si no se da t pero sí vf, usamos: vf = v0 + g * t para despejar t
                t = (vf - v0) / g;
                if (t < 0) {
                ...
                // Si no se da h pero sí t, usamos: h = v0 * t + (1/2) * g * t²
                h = v0 * t + 0.5 * g * t * t;
                // Calculamos vf si no se dio
                if (vf === null) {
                    vf = v0 + g * t;
                }
            } else if (t === null && h !== null && vf !== null) {
                // Si no se da t pero sí h y vf, usamos: vf² = v0² + 2 * g * h para verificar consistencia
                // Luego despejamos t usando: vf = v0 + g * t
                t = (vf - v0) / g;
                if (t < 0) {
                    resultDiv.innerHTML = 'El tiempo calculado es negativo. Revisa los valores ingresados.';
                    return;
                ...
                // Si no se da t ni vf pero sí h, usamos: h = v0 * t + (1/2) * g * t²
                // Esto es una ecuación cuadrática: (1/2) * g * t² + v0 * t - h = 0
                const a = 0.5 * g;
                const b = v0;
                const c = -h;
                ...
                t = (-b + Math.sqrt(discriminant)) / (2 * a); // Tomamos la raíz positiva
                if (t < 0) {
                    t = (-b - Math.sqrt(discriminant)) / (2 * a); // Probamos la otra raíz
                }
                if (t < 0) {
                    resultDiv.innerHTML = 'El tiempo calculado es negativo. Revisa los valores ingresados.';
                    return;
                ...
                // Si no se da vf pero sí h y t, usamos: vf = v0 + g * t
                vf = v0 + g * t;
            }

            // Calculamos el tiempo total si v0 es negativa
            if (v0 < 0) {
                // Tiempo desde la altura máxima hasta el suelo
            ...
            // Convertimos de vuelta a las unidades seleccionadas
            ...
            // Generamos el gráfico de posición vs. tiempo
            const tBase = t * conversions.tiempo[tUnit]; // Tiempo en segundos
            const v0Base = v0 * conversions.velocidad[v0Unit]; // v0 en m/s
            const hBase = h * conversions.longitud[hUnit]; // h en metros
            ...
            for (let i = 0; i <= numPoints; i++) {
                ...
                const position = hBase - (v0Base * time + 0.5 * g * time * time); // Posición y(t) = h - (v0*t + (1/2)*g*t²)
                timePoints.push(time.toFixed(2));
                positionPoints.push(position >= 0 ? position.toFixed(2) : 0); // No mostramos posiciones negativas (suelo)
            }
            ...
            // Generamos el gráfico de trayectoria (x vs y)
            const tBase = t * conversions.tiempo[tUnit]; // Tiempo en segundos
            const v0Base = v0 * conversions.velocidad[v0Unit]; // v0 en m/s
            ...
            for (let i = 0; i <= numPoints; i++) {
                const time = (i / numPoints) * tBase;
                const x = (v0Base * Math.cos(thetaRad)) * time; // x = v0x * t
                const y = (v0Base * Math.sin(thetaRad)) * time - 0.5 * g * time * time; // y = v0y * t - (1/2) * g * t²
                xPoints.push(x.toFixed(2));
                yPoints.push(y >= 0 ? y.toFixed(2) : 0); // No mostramos alturas negativas (suelo)
            }
        ...
        if (subcategory === 'cinetica') {
            ...
            // Generamos un gráfico de energía cinética vs. velocidad
            const vBase = v; // Velocidad en m/s
            const vPoints = [];
            const ekPoints = [];
            const numPoints = 50;
            for (let i = 0; i <= numPoints; i++) {
                const velocity = (i / numPoints) * vBase * 2; // Variamos la velocidad hasta el doble
            ...
            // Generamos un gráfico de energía potencial vs. altura
            const hBase = h; // Altura en metros
            const hPoints = [];
            const epPoints = [];
            const numPoints = 50;
            for (let i = 0; i <= numPoints; i++) {
                const height = (i / numPoints) * hBase * 2; // Variamos la altura hasta el doble
            ...
            // Generamos un gráfico de trabajo vs. ángulo
            const thetaPoints = [];
            const wPoints = [];
            const numPoints = 50;
            for (let i = 0; i <= numPoints; i++) {
                const angle = (i / numPoints) * 180; // Variamos el ángulo de 0° a 180°
        ...
        // Validación de temperatura absoluta (para Kelvin)
        ...
        if (subcategory === 'momento') {
            ...
            // Generamos un gráfico de momento lineal vs. velocidad
            const vBase = v; // Velocidad en m/s
            const vPoints = [];
            const pPoints = [];
            const numPoints = 50;
            for (let i = 0; i <= numPoints; i++) {
                const velocity = (i / numPoints) * vBase * 2 - vBase; // Variamos la velocidad de -v a +v
            ...
            // Velocidad final en colisión inelástica (los objetos se mueven juntos)
        ...
        // Generamos un gráfico de la fuerza gravitacional vs. distancia
        const rBase = r; // Distancia en metros
        const rPoints = [];
        const fPoints = [];
        const numPoints = 50;
        for (let i = 1; i <= numPoints; i++) {
            const distance = (i / numPoints) * rBase * 2; // Variamos la distancia hasta el doble
        ...
        if (subcategory === 'coulomb') {
            ...
            // Generamos un gráfico de la fuerza electrostática vs. distancia
            const rBase = r; // Distancia en metros
            const rPoints = [];
            const fPoints = [];
            const numPoints = 50;
            ...
            // Generamos un gráfico de corriente vs. resistencia
            const rBase = r; // Resistencia en ohmios
            const rPoints = [];
            const iPoints = [];
            const numPoints = 50;
            for (let i = 1; i <= numPoints; i++) {
                const resistance = (i / numPoints) * rBase * 2; // Variamos la resistencia hasta el doble
        ...
        if (subcategory === 'velocidad_onda') {
            ...
            const theta_r = theta_i; // Ley de reflexión: ángulo de incidencia = ángulo de reflexión
